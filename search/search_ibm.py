...
def search_ibm(keyword, source, results, seen_links, url_blacklist):
    if source not in results:
        results[source] = []

    rss_url = "https://newsroom.ibm.com/announcements?pagetemplate=rss"
    ...
    for entry in feed.entries:
        ...
        try:
            ...
            if paragraphs:
                first_p = "\n\n".join(p.get_text() for p in paragraphs[0:2])
            else:
